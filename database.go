...
func (db *DB) StructureCreate() {
	structure := `
	CREATE TABLE IF NOT EXISTS users (id INTEGER NOT NULL PRIMARY KEY, email TEXT NOT NULL UNIQUE, passwd TEXT NOT NULL, name TEXT, surname TEXT, organization TEXT, phone TEXT);
...
func (db *DB) UserAdd(user *User) (int64, error) {
	ret, err := db.DB.NamedExec(`INSERT INTO users (email, passwd, name, surname, organization) VALUES (:email, :passwd, :name, :surname, :organization)`, user)
	if err != nil {
		return -1, err
	}
...
func (db *DB) UserMod(user *User) error {
	_, err := db.DB.NamedExec(`UPDATE users SET passwd=:passwd, name=:name, surname=:surname, organization=:organization WHERE email=:email`, user)
	return err
}
...
func (db *DB) RoomAsignToUser(email string, roomID int64) error {
	//TODO
	return fmt.Errorf("RoomAsignToUser unimplemented")
}
...
func (db *DB) FurnitureGetByNumber(number int64) (Furniture, error) {
	furniture := Furniture{}
	err := db.DB.Get(&furniture, `SELECT * FROM furnitures WHERE number=$1`, number)
