package main

import (
	"os"
	"testing"
)

// not unit test, this is integration test
// to run it: invoke it like that:
// MSERVER= MUSER= MPASS= MTO= go test

var (
	server = os.ExpandEnv("$MSERVER")
	user   = os.ExpandEnv("$MUSER")
	pass   = os.ExpandEnv("$MPASS")
	to     = []string{os.ExpandEnv("$MTO")}
	n, n2  NotifConfig
)

func Init() {
	t := testing.T{}
	if server == "" {
		t.Fatalf("you have to set enviroment vars, invoke it like this:\nMSERVER= MUSER= MPASS= MTO= go test\n")
	}
	n = NotifConfig{
		Server: server,
		User:   user,
		Pass:   pass,
		Port:   587,
		To:     to,
	}
	n2 = n
	n2.Port = 465
}

func TestMail_Send(t *testing.T) {
	Init()
	type args struct {
		n NotifConfig
	}
	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{"timeouting send", args{NotifConfig{Server: "server.com", User: "a", Pass: "b", To: []string{"to"}}}, true},
		{"not existing srv", args{NotifConfig{Server: "rntuylmj320n290n03k093km43209d2.com", User: "a", Pass: "b", To: []string{"to"}}}, true},
		{"simple submission send", args{n}, false},
		{"simple tls send", args{n2}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &Mail{}
			t.Log(tt.args.n)
			if err := m.Send(tt.args.n); (err != nil) != tt.wantErr {
				t.Errorf("Mail.Send() error = %v, wantErr %v", err, tt.wantErr)
			} else {
				t.Log(err)
			}
		})
	}
}
